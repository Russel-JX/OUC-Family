...
* 			所以属性的初始化在创建对象的时候，可在属性定义处初始化，可在自由代码快中初始化或者构造函数中初始化。
...
* 		1.不提供set方法修改成员变量(对final属性，Eclipse是不会自动生成set方法的)；
* 		2.把final属性对象的各个细节copy到新变量中，返回新变量，即每次使用属性的get方法都把属性copy到新的对象返回。（不是地址赋值，地址复制返回的新变量地址就是final属性地址，这样还能修改final对象的值）
*/
